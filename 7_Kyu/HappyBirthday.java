...
public class WrapPresent{
    public static int wrap(int height, int width, int length){
        int min = Math.min(Math.min(height,length), width);
        int max = Math.max(Math.max(height,length), width);
        int med = (height + width + length) - min - max;
        return (2 * med) + (2 * max) + (4 * min) + 20;
    }
}
